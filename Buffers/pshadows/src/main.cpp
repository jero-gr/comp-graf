...
int main() {
	...
	do {
		...
		glEnable(GL_STENCIL_TEST);		// Prendo el stencil test
		glDepthFunc(GL_NEVER);			// Sólo dibujar en el stencil buffer
		
		glStencilFunc(GL_EQUAL,1,~0);	// Que pase siempre con valor 1
		...
		*/
		...
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); // En adelante, que nunca cambien los valores de stencil
		glDepthFunc(GL_LESS);					// Dejar de dibujar en el stencil buffer
		
		// draw objects
		/// @todo: seleccionar la mascara y el valor de referencia adecuado para cada objecto
		
		// El reflejo se debe dibujar dentro de los stencils 1 y 2 (0 < 1,2)
		glStencilFunc(GL_LESS,0,~0);
		drawObject(reflection);
		
		// La sombra se debe dibujar dentro del stencil 2 (2 = 2)
		glStencilFunc(GL_EQUAL,2,~0);
		drawFloor(false);
		
		// El piso se debe dibujar dentro del stencil 1 (1 = 1)
