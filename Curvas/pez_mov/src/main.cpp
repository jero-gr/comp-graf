...
void updateControlPointsAround(Spline &spline, int ctrl_pt) {
	...
	/// Método de Overhausser
	
	glm::vec3 p_i = spline.getControlPoint(ctrl_pt);
	glm::vec3 p_prev = spline.getControlPoint(ctrl_pt-3);
	glm::vec3 p_next = spline.getControlPoint(ctrl_pt+3);
	
	glm::vec3 v_prev = p_i - p_prev;
	glm::vec3 v_prev_norm = glm::normalize(v_prev);
	
	glm::vec3 v_next = p_next - p_i;
	glm::vec3 v_next_norm = glm::normalize(v_next);
	
	glm::vec3 v_i = (glm::length(v_next)*v_prev_norm + glm::length(v_prev)*v_next_norm)/(glm::length(v_next)+glm::length(v_prev));
	
	glm::vec3 p_in = p_i - (glm::length(v_prev) * v_i)/3.f;
	glm::vec3 p_out = p_i + (glm::length(v_next) * v_i)/3.f;
	
	spline.setControlPoint(ctrl_pt-1, p_in);
	spline.setControlPoint(ctrl_pt+1, p_out);
}
...
glm::mat4 getTransform(const Spline &spline, double t) {
	...
	glm::vec3 deriv;
	glm::vec3 pos = spline.at(t,deriv);

	glm::vec3 e_x = glm::normalize(deriv);
	glm::vec3 e_y(0.f,1.f,0.f);
	glm::vec3 e_z = glm::cross(e_x,e_y);
