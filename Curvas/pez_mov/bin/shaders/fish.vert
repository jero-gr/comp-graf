...
void main() {
	vec4 newPosition = vec4(vertexPosition,1.f);
	float sin_t = sin(time*2*3.1415926538);
	newPosition.z += pow(cos( (newPosition.x+1.f)/2.f ),100)*.4 * sin_t;
	newPosition.z += pow(cos( (1.f-newPosition.x)/2.f ),25)*.1 * sin_t;
