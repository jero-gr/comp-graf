...
std::vector<glm::vec2> generateTextureCoordinatesForSkyDome(const std::vector<glm::vec3> &positions) {
	...
	//glm::vec3 centroide = glm::vec3(0.f,0.f,0.f);
	glm::vec3 pos_max = positions[0];
	glm::vec3 pos_min = positions[0];
	
	/// Busco el centroide de la esfera y los valores máximos y mínimos en cada coordenada para normalizar
	for(int i=0;i<positions.size();i++) { 
		//centroide += positions[i];
		if (positions[i][0]>pos_max[0]) pos_max[0] = positions[i][0];
		if (positions[i][1]>pos_max[1]) pos_max[1] = positions[i][1];
		if (positions[i][2]>pos_max[2]) pos_max[2] = positions[i][2];
		
		if (positions[i][0]<pos_min[0]) pos_min[0] = positions[i][0];
		if (positions[i][1]<pos_min[1]) pos_min[1] = positions[i][1];
		if (positions[i][2]<pos_min[2]) pos_min[2] = positions[i][2];
	}
	...
	for(int i=0;i<positions.size();i++) {
		...
		glm::vec2 pos_i = glm::vec2(positions[i][0],positions[i][2]);
		pos_i = glm::normalize(pos_i);
		
		float theta = atan2(pos_i[1],pos_i[0]);
		float s = theta/(2.f*3.14159);
